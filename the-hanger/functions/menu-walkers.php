...
{    
    ...
    function start_lvl( &$output, $depth = 0, $args = array() ) {
    	if (!in_array($this->curItem->ID, $this->noIDS)) {
			...
			$output .= "\n$indent<ul class=\"vertical menu site-main-font\" data-submenu>\n";
		}
    }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
    	$this->curItem = $item;
    	if ( GBT_Opt::getOption('enable_megamenu_' . $item->ID, 0) == true) {
			$this->noIDS[] = $item->ID;
		}
	    if ( (GBT_Opt::getOption('enable_megamenu_' . $item->menu_item_parent, 0) == true) || ( in_array($item->menu_item_parent, $this->noIDS))) {
	    	$this->noIDS[] = $item->ID;
	    	return;
  		}
