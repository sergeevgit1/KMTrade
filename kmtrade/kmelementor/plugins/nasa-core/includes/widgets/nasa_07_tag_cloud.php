...
if (class_exists('WC_Widget')) {
    ...
    add_action('widgets_init', 'nasa_tag_cloud_widget');
    function nasa_tag_cloud_widget() {
        register_widget('Nasa_Widget_Tag_Cloud');
    }

    class Nasa_Widget_Tag_Cloud extends WP_Widget {
        ...
        public function __construct() {
            $widget_ops = array(
                'description' => esc_html__('A cloud of your most used tags.', 'nasa-core'),
                'customize_selective_refresh' => true,
                'title' => 'Tag cloud',
                'show_items' => 'All'
            );
            parent::__construct('nasa_tag_cloud', esc_html__('Nasa Tag Cloud', 'nasa-core'), $widget_ops);
        }
        ...
        public function widget($args, $instance) {
            ...
            if ($tags) {
                ...
                if ($show_items) {
                    echo '<li class="nasa_show_manual" data-delay="500" data-fadein="1"><a data-show="1" class="nasa-show" href="javascript:void(0);">' . esc_html__('+ Show more', 'nasa-core') . '</a><a data-show="0" class="nasa-hidden" href="javascript:void(0);">' . esc_html__('- Show less', 'nasa-core') . '</a></li>';
                }
                echo '</ul>';
            }
        ...
        public function form($instance) {
            ...
            echo '<p>' .
                '<label for="' . $title_id . '">' . esc_html__('Title:', 'nasa-core') . '</label>' .
            ...
            switch (count($taxonomies)) {
                ...
                case 0:
                    echo '<p>' . esc_html__('The tag cloud will not be displayed since there are no taxonomies that support the tag cloud widget.', 'nasa-core') . '</p>';
                    printf($input, '');
                    break;
                ...
                default:
                    printf(
                        ...
                        '<select class="widefat" id="%1$s" name="%3$s">', $id, esc_html__('Taxonomy:', 'nasa-core'), $name
                    );
            ...
            echo '<p><label for="' . esc_attr($title_id) . '">' . esc_html__('Show items (<= 0 ~ All)', 'nasa-core') . '</label>' .
                '<input type="text" class="widefat" id="' . esc_attr($title_id) . '" name="' . esc_attr($this->get_field_name('show_items')) . '" value="' . (isset($instance['show_items']) && is_numeric($instance['show_items']) ? $instance['show_items'] : esc_attr__('All', 'nasa-core')) . '" />' .
            '</p>';
        }
