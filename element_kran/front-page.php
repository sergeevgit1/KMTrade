...
// Значения по умолчанию для секции о компании
$about_defaults = [
    'title' => 'О компании',
...
<!-- Hero блок -->
<section class="hero-section relative overflow-hidden mb-0">
...
<!-- О компании -->
<section class="about-section py-8" style="margin-bottom: <?php echo esc_attr(get_theme_mod('about_spacing', '8')); ?>px;">
...
<!-- Почему выбирают нас -->
<section class="advantages-section py-8" style="margin-bottom: <?php echo esc_attr(get_theme_mod('advantages_spacing', '8')); ?>px;">
    <div class="container mx-auto px-4">
...
<!-- Блог и новости -->
<section class="blog-section py-8" style="margin-bottom: <?php echo esc_attr(get_theme_mod('blog_spacing', '8')); ?>px;">
    <div class="container mx-auto px-4">
...
<!-- Поиск запчастей -->
<section class="how-to-find-section py-8">
    <div class="container mx-auto px-4">
        <!-- Варианты поиска -->
...
<!-- FAQ секция -->
<section class="faq-section bg-gray-50 py-8">
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <?php
            // Массив с вопросами и ответами по умолчанию
            $default_faqs = array(
                array(
                    'question' => 'Как заказать кран?',
                    'answer' => 'Оставьте заявку на сайте или позвоните нам. Наш менеджер рассчитает стоимость и подберет подходящий кран.'
                ),
                array(
                    'question' => 'Какие документы нужны для аренды?',
                    'answer' => 'Для юридических лиц - реквизиты компании. Для физических лиц - паспорт и договор аренды.'
                ),
                array(
                    'question' => 'Работаете ли вы по выходным?',
                    'answer' => 'Да, мы работаем без выходных. Техника доступна для аренды 24/7.'
                ),
                array(
                    'question' => 'Какой срок минимальной аренды?',
                    'answer' => 'Минимальный срок аренды - 4 часа работы крана.'
                ),
                array(
                    'question' => 'Есть ли у вас страховка?',
                    'answer' => 'Да, вся наша техника застрахована. Мы гарантируем безопасность работ.'
                ),
                array(
                    'question' => 'Работаете ли вы в области?',
                    'answer' => 'Да, мы работаем по всей области. Стоимость доставки рассчитывается индивидуально.'
                )
            );

            // Получаем настроенные FAQ из админки или используем значения по умолчанию
            $faqs = array();
            for ($i = 1; $i <= 6; $i++) {
            ...
            // Если нет настроенных FAQ, используем значения по умолчанию
            if (empty($faqs)) {
                $faqs = $default_faqs;
            }
            ...
            $faqs_per_column = ceil($total_faqs / 2);
            $columns = array_chunk($faqs, $faqs_per_column);

            // Выводим первую колонку
            if (!empty($columns[0])) : ?>
                <div class="space-y-4">
                    <?php foreach ($columns[0] as $faq) : ?>
...
<section class="contact-section py-8" style="margin-bottom: <?php echo esc_attr(get_theme_mod('contact_spacing', '8')); ?>px;">
    <div class="container mx-auto px-4">
        <!-- Заголовок секции -->
        <div class="max-w-2xl mx-auto text-center mb-12">
            ...
            <p class="text-xl text-zinc-600">Свяжитесь с нами любым удобным способом</p>
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <div class="bg-white rounded-xl border border-zinc-100 p-8 mb-8">
                    <div class="space-y-6">
                        <!-- Телефон -->
            ...
            <!-- Карта -->
            <div>
                <div class="bg-white rounded-xl border border-zinc-100 p-8 h-full">
                    <h4 class="font-medium text-zinc-900 mb-6">Мы на карте</h4>
...
<script>
    ymaps.ready(function () {
        ...
        var myPlacemark = new ymaps.Placemark([55.751574, 37.573856], {
            hintContent: 'Element Kran',
            balloonContent: '<?php echo esc_js(get_theme_mod('contact_address', $contact_defaults['address'])); ?>'
        }, {
